...
prepareForMSstats <- function(phosphosites, global_evidence, min_match=2, min_global_nonMissing=2) {
  ...
  global_evidence <- global_evidence %>% filter(rowSums(across(matches("Reporter intensity corrected \\d+"), function(x) x > 0)) >= 2)
...
addSitesToNames <- function(genes, ids, positions, amino_acid) {
  ...
  ids_with_sites <- mapply(function(s_genes, s_ids, s_pos,aa) {
    ...
    str_c(s_genes, "_", s_ids, "_", aa, s_pos, collapse=";")
